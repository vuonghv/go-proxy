// A revese proxy that load-balances between two backends using round-robin algo
package main
...
func loadBalancingReverseProxy(target1, target2 *url.URL) *httputil.ReverseProxy {
    ...
    director := func(req *http.Request) {
        var target *url.URL
        // Simple round-robin between two targets
        if targetNum == 1 {
            target = target1
            targetNum = 2
        } else {
        ...
        // see the full code of **NewSingleHostReverseProxy** for an example of
        // doing that
    }
    return &httputil.ReverseProxy{Director: director}
}

// singleJoiningSlahs is stolen from net/http/httputil
...
// joinURLPath is stolen from net/http/httputil
